...
Example:
        ...
        $ python regularKNearestNeighbors2Regression.py
...
# Fitting the K-Nearest Neighbor classification model to the dataset
...
plt.title('K-Nearest Neighbor classification model (Training set)')
plt.xlabel('Age')
...
for i, j in enumerate(np.unique(y_set)):
    plt.scatter(X_set[y_set == j, 0], X_set[y_set == j, 1],
                c=ListedColormap(('red', 'blue'))(i), label=j)
plt.title('K-Nearest Neighbor classification model (Testing set)')
plt.xlabel('Age')
plt.ylabel('Estimated Salary')
plt.legend()
plt.show()
